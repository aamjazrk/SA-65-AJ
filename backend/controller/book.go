...
func CreateBook(c *gin.Context) {
	...
	var book_type entity.Book_type
	...
	if tx := entity.DB().Where("id = ?", book.Emp_ID).First(&employee); tx.RowsAffected == 0 {
...
func ListBook(c *gin.Context) {
	...
	if err := entity.DB().Preload("Shelf").Preload("Role").Preload("Book_type").Preload("Employee").Raw("SELECT * FROM books").Find(&books).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
