...
func SetupDatabase() {
	...
	db.Model(&Book_type{}).Create(&BT1)

	BT2 := Book_type{
		Type: "ELECTRIC ENGINEERING",
	}
	db.Model(&Book_type{}).Create(&BT2)

	BT3 := Book_type{
		Type: "SUPERHERO FANTASY",
	}
	db.Model(&Book_type{}).Create(&BT3)

	BT4 := Book_type{
		Type: "HORROR FICTION",
	}
	db.Model(&Book_type{}).Create(&BT4)

	BT5 := Book_type{
		Type: "DARK AND GRIMDARK FANTASY",
	}
	db.Model(&Book_type{}).Create(&BT5)
	BT6 := Book_type{
		Type: "CONTEMPORARY FANTASY",
	}
	db.Model(&Book_type{}).Create(&BT6)
